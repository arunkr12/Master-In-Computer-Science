...
class ToDoContainer {
  constructor() {
    ...
    this.refreshManager = new RefreshManager(this.todoService, 60000);
  ...
  async addTodoItem(todoData) {
    if (todoData.text.trim()) {
      ...
      if (newTodo) {
        ...
        // Cache the initial version so immediate edits don't trigger false conflicts
        this.concurrencyManager.cacheVersion(newTodo.id, newTodo.version);
        this.updateComponents();
    ...
    try {
      ...
      if (success) {
        ...
        if (
          ...
          typeof this.concurrencyManager.removeVersion === "function"
        ) {
          this.concurrencyManager.removeVersion(id);
        }
        this.updateComponents();
        return true;
      }

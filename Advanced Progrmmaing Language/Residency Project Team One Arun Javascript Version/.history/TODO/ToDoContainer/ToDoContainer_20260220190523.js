...
class ToDoContainer {
  ...
  async editTodoItem(id, newText) {
    if (newText.trim()) {
      ...
      const result = await this.todoService.updateTodo(id, {
        text: newText.trim(),
      }, this.currentUserId);
      ...
      if (result.success) {
        ...
        this.concurrencyManager.logChange(id, 'UPDATE', this.currentUserId, null, result.todo);
        ...
        alert(`⚠️ Conflict: This task was recently modified. Please refresh to see the latest version.`);
  ...
  async toggleTodoItem(id) {
    ...
    if (todo) {
      ...
      if (result.success) {
        // Update cache and local state
        this.concurrencyManager.cacheVersion(id, result.todo.version);
        this.concurrencyManager.logChange(id, 'UPDATE', this.currentUserId, 
          { completed: todo.completed }, 
